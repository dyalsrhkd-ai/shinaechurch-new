  ...
  return (
    ...
    >
      <div style={{ position: 'relative', aspectRatio: '16/9', background: '#0a1628', overflow: 'hidden' }}>
        ...
        />
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(10,22,48,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity 0.2s' }}
...
export default function Sunday() {
  ...
  useEffect(() => {
    getDocs(query(collection(db, 'sundaySermons'), orderBy('order', 'desc')))
      ...
      {loading ? (
        ...
        <>
          ...
          {featured && (
            ...
            >
              ...
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '24px' }}>
                <span style={{ display: 'inline-block', background: '#dc2626', color: '#fff', fontSize: '0.7rem', fontWeight: 700, padding: '3px 10px', borderRadius: '9999px', marginBottom: '10px' }}>주일예배 최신</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '32px' }}>
            {gridSermons.map(s => (
              <SermonCard key={s.id} s={s} onClick={() => setModal(s)} />
            ))}
          </div>
