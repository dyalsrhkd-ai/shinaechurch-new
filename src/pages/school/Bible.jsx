...
const page = schoolContentPages.find(item => item.id === 'school-bible')
...
export default function Bible() {
  ...
  const photos = useMemo(() => galleryItems.filter(item => item.dept === '성경대학'), [galleryItems])
  const videos = useMemo(() => videoItems.filter(item => item.dept === '성경대학'), [videoItems])

  return (
    <SubLayout section="교회 학교" menus={menus} title="성경대학">
      <SchoolPageContent pageState={content} pageId="school-bible" photos={photos} videos={videos} />
    </SubLayout>
  )
}
