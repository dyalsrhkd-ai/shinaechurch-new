...
const page = schoolContentPages.find(item => item.id === 'school-youth')
...
export default function Youth() {
  ...
  const photos = useMemo(() => galleryItems.filter(item => item.dept === '중·고등부'), [galleryItems])
  const videos = useMemo(() => videoItems.filter(item => item.dept === '중·고등부'), [videoItems])

  return (
    <SubLayout section="교회 학교" menus={menus} title="중·고등부">
      <SchoolPageContent pageState={content} pageId="school-youth" photos={photos} videos={videos} />
    </SubLayout>
  )
}
