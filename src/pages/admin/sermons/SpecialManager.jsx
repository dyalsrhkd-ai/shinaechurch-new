...
export default function SpecialManager() {
  ...
  const fetchSermons = async () => {
    setLoading(true)
    try {
      const snap = await getDocs(query(collection(db, 'specialSermons'), orderBy('order', 'desc')))
  ...
  const handleAdd = async () => {
    ...
    setSaving(true)
    try {
      await addDoc(collection(db, 'specialSermons'), {
  ...
  const handleDelete = async (id) => {
    ...
    await deleteDoc(doc(db, 'specialSermons', id))
    fetchSermons()
  }
  ...
  return (
    <div>
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '1.4rem', fontWeight: 900, color: '#0f2040' }}>특별설교 관리</h1>
      ...
      <div style={{ background: '#f8fafc', border: '1px solid #eaecf0', borderRadius: '14px', padding: '20px', marginBottom: '28px' }}>
        <p style={{ fontWeight: 700, fontSize: '0.875rem', color: '#0f2040', marginBottom: '14px' }}>특별설교 등록</p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          <input
            value={form.title}
            onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
            placeholder="설교 제목 *"
            style={{ padding: '10px 12px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '0.875rem' }}
          />
          <input
            value={form.ref}
            onChange={e => setForm(f => ({ ...f, ref: e.target.value }))}
            placeholder="성경 구절 (예: 창 3:1-10)"
            style={{ padding: '10px 12px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '0.875rem' }}
          />
          <input
            value={form.date}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginTop: '14px' }}>
          {previewId && (
            <img
              ...
              alt=""
              style={{ width: '140px', height: '79px', objectFit: 'cover', borderRadius: '8px', border: '1px solid #eaecf0', flexShrink: 0 }}
            />
          )}
          <button
            onClick={handleAdd}
            disabled={saving}
            style={{ padding: '10px 28px', background: '#7c3aed', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem' }}
          >
            {saving ? '저장 중...' : '등록'}
          </button>
        </div>
